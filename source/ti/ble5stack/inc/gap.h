...
/// @cond NODOC
  ...
  #define TGAP_GGS_PARAMS             102 //!< GGS TestCodes and parameters - puts GGS into a test mode (paramValue maintained by GGS).  Also used to disable RPAO.
#else
...
/// @brief GAP Device Privacy Mode Callback Function
typedef uint8 (*pfnGapDevPrivModeCB_t)
(
  uint8  addrType,                    //!< @ref Addr_type of device to check
  uint8 *pAddr                        //!< address of device to check if Device Privacy mode is permissible
);
 ...
 */
 ...
 /**
  ...
  /**
   * @brief       Set the Privacy Mode of the requested device to the
   *              desired mode.
   *
   * @param       addrType - remote device's identity @ref Addr_type
   * @param       pAddr    - pointer to the address
   * @param       mode     - GAP_PRIVACY_MODES to use for this device
   *
   * @return      SUCCESS: match,<BR>
   *              FAILURE: don't match,<BR>
   *              INVALIDPARAMETER: parameters invalid<BR>
   */
  extern bStatus_t GAP_SetPrivacyMode( uint8 addrType, uint8 *pAddr, uint8 mode );
