...
void BMA250E_IDTest(void)
{
  ...
  {
    if(rxbuf[0] == 0xF9)
    HwUARTWrite("ReadID_OK",10);
  }
  else
  {
    HwUARTWrite("ReadID_ERR",11);
  }
}
...
bool BMA250E_GetData(void)
{
  ...
  HwUARTPrintf("X:%d Y:%d Z:%d",buffer[1],buffer[3],buffer[5]);
  Display_print3(dispHandle, 1, 0, "X:%d Y:%d Z:%d",buffer[1],buffer[3],buffer[5]);
