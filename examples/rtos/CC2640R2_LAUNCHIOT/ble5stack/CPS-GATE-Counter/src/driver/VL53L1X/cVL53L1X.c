...
bool VL_init(bool io_2v8){
  ...
  setDistanceMode(VL_Long);
  setMeasurementTimingBudget(33000);//for long mode use at least 33000
...
void writeReg(uint16_t reg, uint8_t value){
	...
	last_status = I2C_transfer(I2CHandle, &i2cTransaction);
}
...
uint16_t readReg16Bit(uint16_t reg){
  uint16_t value;
	...
	i2cTransaction.writeBuf = buf;
	i2cTransaction.writeCount = 2;
	i2cTransaction.readBuf = Rdbuf;
	i2cTransaction.readCount = 2;
	i2cTransaction.slaveAddress = Slave_Addr;
	i2cTransaction.arg = NULL;
	last_status = I2C_transfer(I2CHandle, &i2cTransaction);
	value = (uint16_t)(Rdbuf[1]|(Rdbuf[0]<<8));
...
uint32_t readReg32Bit(uint16_t reg){
  uint32_t value;
	...
	i2cTransaction.arg = NULL;
	last_status = I2C_transfer(I2CHandle, &i2cTransaction);
	value = (uint32_t)(Rdbuf[3]|(Rdbuf[2]<<8)|(Rdbuf[1]<<16)|(Rdbuf[0]<<24));
...
void readResults(){
  ...
  buf[1] = ( RESULT__RANGE_STATUS       & 0xFF); // reg low byte
  ...
  results.range_status = Rdbuf[0];

  //[1]Wire.read(); // report_status: not used

  //[2]results.stream_count = Wire.read();
  results.stream_count = Rdbuf[2];

  //[3]results.dss_actual_effective_spads_sd0  = (uint16_t)Wire.read() << 8; // high byte
  //[4]results.dss_actual_effective_spads_sd0 |=           Wire.read();      // low byte
  results.dss_actual_effective_spads_sd0 = (uint16_t)(Rdbuf[3]<<8)|Rdbuf[4];

  //[5]Wire.read(); // peak_signal_count_rate_mcps_sd0: not used
  //[6]Wire.read();

  //[7]results.ambient_count_rate_mcps_sd0  = (uint16_t)Wire.read() << 8; // high byte
  //[8]results.ambient_count_rate_mcps_sd0 |=           Wire.read();      // low byte
  results.ambient_count_rate_mcps_sd0 = (uint16_t)(Rdbuf[7]<<8)|Rdbuf[8];

  //[9]Wire.read(); // sigma_sd0: not used
  //[10]Wire.read();

  //[11]Wire.read(); // phase_sd0: not used
  //[12]Wire.read();

  //[13]results.final_crosstalk_corrected_range_mm_sd0  = (uint16_t)Wire.read() << 8; // high byte
  //[14]results.final_crosstalk_corrected_range_mm_sd0 |=           Wire.read();      // low byte
  results.final_crosstalk_corrected_range_mm_sd0 = (uint16_t)(Rdbuf[13]<<8)|Rdbuf[14];

  //[15]results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0  = (uint16_t)Wire.read() << 8; // high byte
  //[16]results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           Wire.read();      // low byte
  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 = (uint16_t)(Rdbuf[15]<<8)|Rdbuf[16];
}
