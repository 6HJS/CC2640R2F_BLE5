...
static void MicroEddystoneBeacon_init(void)
{
  ...
  uble_stackInit(UB_ADDRTYPE_PUBLIC, NULL, uBLEStack_eventProxy, RF_TIME_CRITICAL);
#endif /* FEATURE_STATIC_ADDR */
...
static void MicroEddystoneBeacon_startBroadcast(void)
{
  ...
  uble_getParameter(UB_PARAM_TXPOWER, &tempPower);
...
//    ugap_bcastSetDuty(100, 50); /* OnTime: 10 sec, OffTime: 5 sec */
      ...
      uble_setParameter(UB_PARAM_ADVINTERVAL, sizeof(uint16), &param16);
...
static void MicroEddystoneBeacon_bcast_advPrepareCB(void)
{
  ...
  uble_setParameter(UB_PARAM_TIMETOADV, sizeof(timeToAdv), &timeToAdv);
}
